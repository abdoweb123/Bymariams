            ...
            <div class="col-sm-12 mb-3">
                ...
                <select class="rounded-0 form-select select2" name="country_id" id="country_id" required oninvalid="this.setCustomValidity(@lang('trans.this_field_required'))" title="@lang('trans.this_field_required')">
                    ...
                    @foreach(allCountries() as $country)
                        <option value="{{$country->id}}" data-code="{{$country->country_code}}" {{ old('country_id', $address->country_code == $country->country_code ? 'selected' : '') }}>{{ $country['title_'.lang()] }}</option>
                    @endforeach
                </select>

            </div>
            <div id="otherCountries" class="row" style="{{$address->country_code !== 'BH' ? 'display:block' : 'display:none' }}">
                <div class="col-sm-12 mb-3">
                    <label for="" class="text-secondary">@lang('trans.city')   <span class="red-label">*</span>
                    </label>
                    <input type="text" class="form-control rounded-0" name="city" id="" value="{{ $address ? $address->city : old('city', '') }}" required oninvalid="this.setCustomValidity(@lang('trans.this_field_required'))" title="@lang('trans.this_field_required')">
                </div>
                <div class="col-sm-12 mb-3">
                    <label for="" class="text-secondary">@lang('trans.state')  (@lang('trans.optional'))
                    </label>
                    <input type="text" class="form-control rounded-0" name="state" id="" value="{{ $address ? $address->state : old('state', '') }}">
                </div>
                <div class="col-sm-12 mb-3">
                    <label for="" class="text-secondary">@lang('trans.street_address') <span class="red-label">*</span>
                    </label>
                    <input type="text" class="form-control rounded-0 mb-3" name="address1" id="" placeholder="@lang('trans.address1')" value="{{old('address1',$address ? $address->address1:'')}}" required oninvalid="this.setCustomValidity(@lang('trans.this_field_required'))" title="@lang('trans.this_field_required')">
                    <input type="text" class="form-control rounded-0" name="address2" id="" placeholder="@lang('trans.address2')" value="{{old('address1',$address ? $address->address2:'')}}">
                </div>
            </div>

            <div id="bahrain"  style="{{$address->country_code == 'BH' ? 'display:block' : 'display:none' }}">
                <div class="row">
                    <div class="col-sm-12 col-md-6 mb-3">
                        ...
                        <select class="rounded-0 form-select" name="region_id" id="region_id" required oninvalid="this.setCustomValidity(@lang('trans.this_field_required'))" title="@lang('trans.this_field_required')">
                            ...
                            {{--                                                                                              <option value="{{ old('region_id', $address ? ($address->region_id ?? '') : '') }}" selected>{{$address ? ($address->region_id ? $address->region['title_'.lang()] : '') : ''}}</option>--}}
                            @php
                                $selectedRegionId = old('region_id', $address ? ($address->region_id ?? '') : '');
                            @endphp
                    ...
                    <div class="col-sm-6 col-md-6 mb-3">
                        ...
                        <input type="number" class="form-control rounded-0" name="block" min="1" value="{{ $address ? $address->block : old('block', '') }}" required oninput="this.value = this.value < 1 ? 1 : this.value" oninvalid="this.setCustomValidity(@lang('trans.this_field_required'))" title="@lang('trans.this_field_required')">
                    </div>
                    <div class="col-sm-6 col-md-6 mb-3">
                        <label for="" class="text-secondary">@lang('trans.road')
                        </label>
                        <input type="number" class="form-control rounded-0" name="road" min="1" value="{{ $address ? $address->road : old('road', '') }}" required oninput="this.value = this.value < 1 ? 1 : this.value" oninvalid="this.setCustomValidity(@lang('trans.this_field_required'))" title="@lang('trans.this_field_required')">
                    </div>
                    <div class="col-sm-6 col-md-6 mb-3">
                        <label for="" class="text-secondary">@lang('trans.building_no')
                        </label>
                        <input type="number" class="form-control rounded-0" name="building_no" min="1" value="{{ $address ? $address->building_no : old('building_no', '') }}" required oninput="this.value = this.value < 1 ? 1 : this.value" oninvalid="this.setCustomValidity(@lang('trans.this_field_required'))" title="@lang('trans.this_field_required')">
                    </div>
                    <div class="col-sm-6 col-md-6 mb-3">
                        <label for="" class="text-secondary">@lang('trans.floor_no')
                        </label>
                        <input type="number" class="form-control rounded-0" name="floor_no" min="1" value="{{ $address ? $address->floor_no : old('floor_no', '') }}" required oninput="this.value = this.value < 1 ? 1 : this.value" oninvalid="this.setCustomValidity(@lang('trans.this_field_required'))" title="@lang('trans.this_field_required')">
                    </div>
                    <div class="col-sm-6 col-md-6 mb-3">
                        <label for="" class="text-secondary">@lang('trans.apartment')
                        </label>
                        <input type="number" class="form-control rounded-0" name="apartment" min="1" value="{{ $address ? $address->apartment : old('apartment', '') }}" oninput="this.value = this.value < 1 ? 1 : this.value">
                    </div>
                    <div class="col-sm-6 col-md-6 mb-3">
                        <label for="" class="text-secondary">@lang('trans.apartmentType')
                        </label>
                        <input type="text" class="form-control rounded-0" name="apartmentType" value="{{ $address ? $address->apartmentType : old('apartmentType', '') }}">
                    </div>
                    <div class="col-sm-6 col-md-6 mb-3">
                        <label for="" class="text-secondary">@lang('trans.additional_directions')
                        </label>
                        <input type="text" class="form-control rounded-0" name="additional_directions" value="{{ $address ? $address->additional_directions : old('additional_directions', '') }}">
                    </div>
                </div>
            </div>
